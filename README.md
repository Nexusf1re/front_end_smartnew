...
```
GET /tenance/reports/performance-indicator
```
...
**Rota:** `GET /tenance/reports/performance-indicator`
...
## Deploy com Docker/Nginx (3 instâncias)

Esta aplicação está pronta para rodar 3 instâncias independentes atrás de um Nginx, com cada instância acessível via um subdomínio distinto.

### Arquivos adicionados
- `Dockerfile` (build de produção com output standalone)
- `docker-compose.yml` (app1, app2, app3 e nginx)
- `nginx/nginx.conf` (roteia app1/app2/app3)
- `.dockerignore`
- `scripts/sync.ps1` (Windows) e `scripts/sync.sh` (Linux)
...
Acessos locais (usando `*.localhost`):
- http://app1.localhost
- http://app2.localhost
- http://app3.localhost

Se necessário, adicione entradas ao hosts apontando para `127.0.0.1`.

### Sincronização automática (demo)
Rebuild e redeploy automático em todas as instâncias quando houver mudança no código:
...
### Deploy simples na DigitalOcean (Droplet)
1) Criar Droplet Ubuntu e instalar Docker/Compose (ver doc oficial). Depois:
```bash
docker compose up -d --build
```

2) DNS: criar A records para o IP do droplet:
- `app1.seu-dominio.com`, `app2.seu-dominio.com`, `app3.seu-dominio.com`

3) Ajustar `server_name` em `nginx/nginx.conf` para seus domínios e reiniciar Nginx:
```bash
docker compose restart nginx
```

Observações:
- Cada subdomínio aponta para uma instância específica (sem balanceamento), atendendo ao requisito de URLs distintas.
- Em produção real, prefira CI/CD para build/push de imagens em vez de rebuild no servidor.

---

## CI/CD com GitHub Actions

O pipeline faz build da imagem Docker, publica no GHCR e realiza `docker compose pull && up -d` no Droplet via SSH.

### Arquivos
- `.github/workflows/deploy.yml` – workflow de build & deploy
- `docker-compose.prod.yml` – usa uma única imagem (`${IMAGE}`) para `app1`, `app2`, `app3`
- `.env.example` – exemplo de `.env` para o servidor (define `IMAGE`)

### Secrets necessários (GitHub → Settings → Secrets and variables → Actions)
- `DO_HOST` – IP do droplet
- `DO_USER` – usuário SSH (ex: `root`)
- `DO_SSH_KEY` – chave privada SSH (PEM) para acessar o droplet
- `REMOTE_PATH` – diretório do repositório no droplet (ex: `/opt/smartnew`), contendo `docker-compose.prod.yml` e `nginx/nginx.conf`
- Opcional (se imagem no GHCR for privada):
  - `GHCR_USERNAME` – usuário do GitHub
  - `GHCR_TOKEN` – token com `read:packages`

> Dica: Você pode tornar o pacote do GHCR público e pular o login no servidor.

### Passos iniciais no Droplet (uma única vez)
```bash
sudo apt-get update && sudo apt-get install -y docker.io docker-compose-plugin
sudo mkdir -p /opt/smartnew && cd /opt/smartnew
# Clonar o repositório (ou copiar os arquivos necessários)
git clone https://github.com/<owner>/<repo>.git .
cp .env.example .env  # ajuste IMAGE se quiser fixar
```

### Fluxo de deploy
1. Push na branch `main-docker` dispara o workflow.
2. O workflow publica `ghcr.io/<owner>/<repo>:latest`.
3. SSH no droplet, atualiza `.env` com `IMAGE` e executa:
   - `docker compose -f docker-compose.prod.yml pull`
   - `docker compose -f docker-compose.prod.yml up -d --remove-orphans`

### Por que mudar algo para CI/CD?
- Mantemos `docker-compose.yml` para uso local (build no host local).
- Para produção, `docker-compose.prod.yml` usa a mesma imagem nas 3 instâncias, construída pelo CI – builds são reprodutíveis e rápidos no deploy.
